...
def main():
    ...
    try:
        ...
        commit_result = subprocess.run(
            ["git", "commit", "-m", f"feat: 更新{date.today()}仪表盘数据"],
            cwd=str(BASE_DIR), capture_output=True, text=True
        )
        if commit_result.returncode != 0:
            out = commit_result.stdout + commit_result.stderr
            if "nothing to commit" in out or "no changes" in out:
                print("✓ 无变更需要提交（数据已是最新）")
            else:
                print(f"⚠ git commit 失败: {out[:200]}")
        else:
            push_result = subprocess.run(["git", "push", "origin", "master"],
                                         cwd=str(BASE_DIR), capture_output=True, text=True, timeout=30)
            if push_result.returncode == 0:
                print("✓ 已推送到 GitHub: https://github.com/RGarvel/ZSA4-inspired")
            else:
                print(f"⚠ git push 失败: {push_result.stderr[:200]}")
    except subprocess.TimeoutExpired:
        print("⚠ git push 超时，跳过推送")
